...
struct frame
{
    ...
    void displayFrame()
    {
        for(int a=0;a<30;a++)
        {
            ...
            for(int b=0;b<30;b++)
            {
                SetConsoleTextAttribute(hConsole,fBuffer[a][b]);
                for(int c=1;c<=2;c++)
                {
                    std::cout << fBuffer2[a][b];
                }
            }
        }
        SetConsoleTextAttribute(hConsole,FGWHITE);
    ...
    void updateFrame()
    {
        ...
        for(plot* p:plots)
        {
            ...
            {
                fBuffer[(int)c.y][(int)c.x] = p->colour;
                fBuffer2[(int)c.y][(int)c.x] = p->graphic;
            }
        }
...
void clearDisplay(int x = NULL,int y = NULL)
{
